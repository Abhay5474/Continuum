...
public class GatewayService {
    ...
    public GatewayDtos.ChatResponse chat(String developerId, GatewayDtos.ChatRequest req) {
        ...
        // Provider preference: a developer's OWN configured providers come first
        // (their keys, their request), ordered by the measured-stats scorer
        // (Extension 2, reused), with platform-available providers as fallback.
        SelectionResult selection = selectionEngine.select(canonical, RoutingPolicy.of(mode));
        Map<String, Integer> providerRank = buildProviderOrder(developerId, selection);
        ...
        Map<String, String> devKeys = resolveKeys(developerId, chain);
     ...
     */
    private Map<String, Integer> buildProviderOrder(String developerId, SelectionResult selection) {
